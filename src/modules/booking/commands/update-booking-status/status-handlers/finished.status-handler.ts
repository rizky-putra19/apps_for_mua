...
@Injectable()
export class FinishedHandler extends StatusHandler {
  ...
  async handle(
    ...
    try {
      ...
      const status = await bookingRepo.findByStatus(BookingStatus.FINISHED);
      ...
      if (bookingProps.status.status != BookingStatus.WORK_IN_PROGRESS) {
        throw new Error();
      }
